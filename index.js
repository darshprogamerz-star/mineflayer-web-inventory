/**
 * Ultimate Autonomous Minecraft Companion & AI Brain (All Features 100% Intact)
 * Version: 14.0.0-Titan-Ultimate-Complete
 */
...
/**
 * Native Gemini AI Function (No external SDK, 100% reliable)
 */
async function askAiBrain(promptText, botStatus) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return "Haan boss, batao kya karna hai?";
...
if (DISCORD_TOKEN) {
  ...
  discordClient.once('ready', async () => {
    ...
    if (DISCORD_CHANNEL_ID) {
      ...
      if (discordChannel) discordChannel.send('🟢 **Titan Ultimate AI Agent online & connected!**');
    }
  });
}
  ...
  app.get('/', (req, res) => {
    res.send(`
      ...
      <body>
        <div class="panel">
          <div class="top-bar">
            <div class="title">🤖 Titan Ultimate Agent</div>
    ...
    for (const id in bot.entities) {
      ...
      if (e.type === 'player' || e.type === 'mob') {
        ...
        if (dist <= 28) {
          ...
          nearby.push({ name: entityName, type: e.type, x: e.position.x, z: e.position.z });
        }
      }
    }
    io.emit('radar', {
...
/**
 * Main Daemon Lifecycle (All In-game & Discord Commands Restored)
 */
if (require.main === module) {
  function launchBot() {
    ...
    bot.once('spawn', () => {
      console.log(`[AGENT JOINED] ${bot.username} entered server with Complete Features & AI.`);
    ...
    discordClient.on('messageCreate', async (msg) => {
      ...
      if (content === '!status') {
        ...
        return msg.reply(`📊 **Bot Status:**\n❤️ Health: ${hp}/20 | 🍖 Hunger: ${food}/20\n📍 Position: ${pos}\n🛡️ Guard: ${botState.guardMode ? 'ON' : 'OFF'}\n🚶 AFK: ${botState.antiAfk ? 'ON' : 'OFF'}`);
      }
      if (content === '!afk') {
        if (botState.antiAfk) { stopAntiAfk(bot); msg.reply('🚶 Anti-AFK Wander OFF.'); }
        else { startAntiAfk(bot); msg.reply('🚶 Anti-AFK Wander ON.'); }
        return;
      }
      if (content === '!guard') {
        if (botState.guardMode) { stopGuardMode(bot); msg.reply('🛡️ Guard Mode OFF.'); }
        else { startGuardMode(bot); msg.reply('🛡️ Guard Mode ON.'); }
        return;
      }
      if (content === '!stop') {
        botState.followingPlayer = null;
        stopAntiAfk(bot); stopGuardMode(bot); stopFishing(bot);
        bot.clearControlStates(); bot.pathfinder.stop(); bot.pvp.stop();
        return msg.reply('🛑 Stopped all actions.');
      }
    ...
    // In-game Commands & Restored Full Features + AI
