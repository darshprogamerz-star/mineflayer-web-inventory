 ...
 * VERSION: 33.0.0 (ULTIMATE RENDER.COM EDITION)
 * ============================================================================
 * Included Systems:
 ...
 * - Autonomous Routines: Guard Mode, Auto-Farm, Auto-Fish, House Builder
 * - Dual-Way Discord Synchronizer (!ai, !status, !say, !craft, !netherite)
 ...
 * - AUTONOMOUS NETHERITE PIPELINE (Portal Build + Ancient Debris Mining)
 * - RENDER.COM PORT BINDING OPTIMIZATION
 * ============================================================================
 */
...
app.get('/', (req, res) => {
  res.send('🎮 Titan Bot is running!');
});

app.get('/health', (req, res) => {
  res.json({ status: 'ok', bot: global.bot ? 'connected' : 'connecting' });
});

// CRITICAL: Render port binding - turant listen karo
...
async function equipBestTool(bot, targetBlock) {
  ...
  if (blockName.includes('ore') || blockName.includes('stone') || 
      blockName.includes('cobble') || blockName.includes('deepslate') || 
      blockName.includes('obsidian') || blockName.includes('debris')) {
  ...
  } else if (blockName.includes('wheat') || blockName.includes('carrots') || 
...
async function runAutoSmelter(bot) {
  ...
  const rawOres = bot.inventory.items().filter(i => 
    ...
    i.name.includes('raw_copper') || i.name.includes('_ore')
  );
  ...
  if (chestBlock) {
    try {
      ...
      const valuables = bot.inventory.items().filter(i => 
        i.name.includes('diamond') || i.name.includes('gold') || 
        i.name.includes('iron') || i.name.includes('emerald') || 
        i.name.includes('debris')
      );
...
async function smartGatherAndCraft(bot, targetItemName, count = 1) {
  ...
  async function craftPlanksIfRequired() {
    ...
    if (totalPlanks < 4) {
      ...
      if (logs) {
        const plankRecipe = bot.recipesAll(
          mcData.itemsByName[`${logs.name.replace('_log', '')}_planks`]?.id || 
          mcData.itemsByName['oak_planks'].id, null, 1
        )[0];
        if (plankRecipe) {
          try {
  ...
  try {
    if (
      targetItemName.includes('wood') || targetItemName.includes('plank') ||
      targetItemName.includes('stick') || targetItemName.includes('chest') ||
      targetItemName.includes('crafting_table') || targetItemName.includes('pickaxe') ||
      targetItemName.includes('sword') || targetItemName.includes('axe')
    ) {
...
async function runFarmLoop(bot) {
  ...
  const mcData = require('minecraft-data')(bot.version);
  const cropIds = ['wheat', 'carrots', 'potatoes', 'beetroots']
...
async function executeHouseBuild(bot) {
  const getMat = () => bot.inventory.items().find(i => 
    i.name.includes('plank') || i.name.includes('cobble') || 
...
async function startNetheritePipeline(bot) {
  ...
  if (obsidianCount < 10) {
    bot.chat("❌ Obsidian kam hai! 10 obsidian chahiye.");
    botState.netheritePipeline = false;
    botState.portalBuilding = false;
    return;
  }
  
  // Check flint and steel
  ...
  bot.chat("✅ Portal ready! Ab Nether me ja sakte ho.");
  botState.portalBuilding = false;
  botState.netheritePipeline = false;
}
...
app.post('/api/action', async (req, res) => {
  ...
  if (!bot?.entity) return res.json({ success: false, error: 'Bot not connected' });
...
function launchBot() {
  ...
  discordClient.on('messageCreate', async (msg) => {
    ...
    if (content === '!status') {
      return msg.reply(`📊 HP: ${Math.round(bot.health)}/20 | Guard: ${botState.guardMode ? 'ON' : 'OFF'}`);
    }
  ...
  bot.on('messagestr', async (message) => {
    ...
    if (cmd === 'come' || cmd === 'follow') {
      stopAntiAfk();
      botState.followingPlayer = args[1] || '';
      ...
      smartGatherAndCraft(bot, args[1].toLowerCase(), parseInt(args[2]) || 1);
    }
    else if (cmd === 'mine' && args[1]) {
      mineBlocks(bot, args[1].toLowerCase(), parseInt(args[2]) || 1);
    }
    else if (cmd === 'sort') {
      sortAndCleanInventory(bot);
    }
      ...
      botState.autoSmelt = !botState.autoSmelt;
      if (botState.autoSmelt) runAutoSmelter(bot);
      bot.chat(`🔥 Smelter: ${botState.autoSmelt ? 'ON' : 'OFF'}`);
    }
      ...
      bot.chat(`🛡️ Guard: ${botState.guardMode ? 'ON' : 'OFF'}`);
    }
    else if (cmd === 'afk') {
      botState.antiAfk ? stopAntiAfk() : startAntiAfk(bot);
    }
      ...
      botState.autoFarm = !botState.autoFarm;
      if (botState.autoFarm) runFarmLoop(bot);
      bot.chat(`🌾 Farm: ${botState.autoFarm ? 'ON' : 'OFF'}`);
    }
    else if (cmd === 'deposit') {
      dumpToChest(bot);
    }
      ...
      bot.chat("Sari inventory drop!");
    }
    else {
      if (lower.includes('nokar') || lower.includes('bot')) {
        const prompt = actualText.replace(/^(nokar|bot)\s*/i, '');
  ...
  bot.on('end', () => {
    console.log('[RECONNECT] 10s me retry...');
...
setInterval(() => {
  const bot = global.bot;
  if (bot?.entity) {
    console.log(`[STATUS] HP: ${bot.health}/20 | Pos: ${Math.round(bot.entity.position.x)},${Math.round(bot.entity.position.y)},${Math.round(bot.entity.position.z)}`);
  }
}, 60000);
